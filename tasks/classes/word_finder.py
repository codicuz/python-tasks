class word_finder():
    ...
    def countWord(self):
        ...
        # все числа >= 0 значит слово tinkoff составлено, и букв на составление слова tinkoff хватило, увеличиваем счетчик слова
        # вычитаем слово tinkoff еще раз, получаем
        # {'t': 0, 'i': 0, 'n': 0, 'k': 0, 'o': 0, 'f': 0}
        # все числа >= 0 значит слово tinkoff составлено, и букв на составление слова tinkoff хватило, увеличиваем счетчик слова
        # вычитаем слово tinkoff еще раз, получаем
        # {'t': -1, 'i': -1, 'n': -1, 'k': -1, 'o': -1, 'f': -2}
        # в словаре есть отрицательные значения, это свидетельствует о том, что букв для составления слова tinkoff не хватило,
        # счетчик слова не увеличиваем
        def check_letter_counter():
            # В цикле бежим по значениям счетчика букв, как только встречаем отрицательное значение, выходим и возвращаем False
        ...
        # Считаем количество возможных составлений слова Tinkoff. Считаем, что слово Tinkoff возможно составить столько раз, 
        # пока счетчик интересующих нас букв >= 0. Как только счетчик хотя бы одной буквы становится отрицательным числом, 
        # считаем, что слово Tinkoff составить не возможно.
